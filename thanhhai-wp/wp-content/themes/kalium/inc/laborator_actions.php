...
function kalium_wp_enqueue_scripts() {
	...
	// Single Post
	if ( is_single() ) {
		wp_enqueue_script( 'jquery-', kalium()->assetsUrl( 'js/jquery.ba-throttle-debounce.js' ), null, null, true );
...
function kalium_register_required_plugins() {
	...
	if ( is_shop_supported() ) {
		$plugins[] = array(
			...
			'slug'     => 'prdctfltr',
			'source'   => get_template_directory() . '/inc/thirdparty-plugins/prdctfltr.zip',
			'required' => false,
			'version'  => '6.5.8',
		);
